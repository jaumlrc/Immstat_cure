...
compare_clinical_results_to_referece <- function(input_table, output_prefix_dir) {
  ...
  png(paste(output_prefix_dir, "percentages_with_NA_HV_short.png", sep = "."), width = 1000, height = 1000, res = 300)
  print(ggplot(normal_levels_recovery_wide_HV2_to_plot_melt_split, aes(x=Country_Class, y=Perecentage, fill=Result)) + geom_col() + ylim(0,1) +
          theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_fill_manual(values = c("#56B4E9","#009E73","#E69F00",  "gray"), breaks = c("low","Normal","high", "NA")))
  dev.off()
  ...
  png(paste(output_prefix_dir, "percentages_with_NA.png", sep = "."), width = 1000, height = 1000, res = 300)
  print(ggplot(normal_levels_recovery_wide2_to_plot_na_meltt_split, aes(x=Country_Class, y=Perecentage, fill=Result)) + geom_col()  + ylim(0,1) +
          theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_fill_manual(values = c("#56B4E9","#009E73","#E69F00",  "gray"), breaks = c("low","Normal","high", "NA")))
  dev.off()
...
statistical_comparison_all <- function(input_table, output_plots) {
  ...
  dashed_lines <- data.frame(
    ...
    yintercept = c(11, 2.5, 1, 0.66, 150, 0, 0, 3, 0 ) 
  )
  
  dashed_lines_max <- data.frame(
    variable = c("Hemoglobin", "WBCells", "Neutrophil", "Lymphocyte", "Platelets", "ALT_Results", "Creatinine", "Albumin", "Total_Bilirubin"),
    yintercept = c(16, 10, 6.6, 3.5, 450, 40, 1.4, 5, 1 ) 
  )
...
run_splsda_several_values <- function(input_table, trait,  vector_traits_evalute, country, outname) {
  ...
  Temp_Hep_scores2wide_final1 <- Temp_Hep_scores2wide[which(Temp_Hep_scores2wide$Class=="PLSDA"),]

  png(paste(outname, "final1_predictions.png", sep ="" ), width = 1500, height = 1200, res = 300)
  print(ggplot(Temp_Hep_scores2wide_final1) + 
...
#This is the file used as the metadata table
write.csv(All_countries_clinical_Legendplex_data, "All_patients_all_data.scv", row.names = FALSE, quote = FALSE)
...
stats_uganda <- statistical_comparison_all(All_countries_data_males_uganda, "immstat_Male_data.dir/Statistical_analysis_all/Uganda/Uganda_males_statistical")
stats_sudan <- statistical_comparison_all(All_countries_data_males_sudan, "immstat_Male_data.dir/Statistical_analysis_all/Sudan/Sudan_males_statistical")
...
pca_and_umap_funcion(All_countries_data_males, "immstat_Male_data.dir/PCA_analysis_all/All/All_males_PCA", 8)
...
input_table <- All_countries_clinical_Legendplex_data
output_prefix_dir <-  "immstat_Male_data.dir/All_data"

compare_clinical_results_to_referece2 <- function(input_table, output_prefix_dir) {
  #According to the project protocol, the reference values were:
  #Albumin 3 to 5g
  # Haemoglobin in men 11.0 – 16.0
  # Platelets adults 150–450
  ...
  temp1_Eth <- input_table2_to_count[which(input_table2_to_count$Country=="Ethiopia" & input_table2_to_count$Class=="V2"),]
  View(temp1_Eth)
...
dashed_lines <- data.frame(variable = c("Hemoglobin", "WBCells", "Neutrophil", "Lymphocyte", "Platelets", "ALT_Results", "Creatinine", "Albumin", "Total_Bilirubin"),
  yintercept = c(11, 2.5, 1, 0.66, 150, 0, 0, 3, 0 )   # Different y-values for each facet
)
...
dashed_lines_max <- data.frame(variable = c("Hemoglobin", "WBCells", "Neutrophil", "Lymphocyte", "Platelets", "ALT_Results", "Creatinine", "Albumin", "Total_Bilirubin"),
  yintercept = c(16, 10, 6.6, 3.5, 450, 40, 1.4, 5, 1 )  # Different y-values for each facet
)
dashed_lines$variable <- factor(dashed_lines$variable, c( "Albumin", "Hemoglobin",
                                                          "Platelets", "Creatinine", "Total_Bilirubin", "ALT_Results", "WBCells", "Neutrophil", 
                                                          "Lymphocyte"))
